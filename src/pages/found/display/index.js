...
var _display = {
    ...
    get_data : function(){
        var submit_data = _patent.getUrlParam('paper_uuid');
        var _this = this;
        _patent.request({
            //发data到服务器地址
            url : 'http://10.210.17.103:8000/api/paper/detail/'+submit_data,
            method : 'get',
    ...
    get_date : function(res){
        ...
        date["applicarionDate"] = res.applicationDate.raw.substr(0,4)+'/'+res.applicationDate.raw.substr(4,2)+'/'+res.applicationDate.raw.substr(6,2);
        date["publishedDate"] = res.publishedDate.raw.substr(0,4)+'/'+res.publishedDate.raw.subtr(4,2)+'/'+res.publishedDate.raw.substr(6,2);
        return date;
    },
